"""
XAA + Agent as a Principal UI — single Flask file.
...
Flow when you click "Run XAA Flow":
    ...
    4. Agent exchanges id_token at Okta → ID-JAG
    5. Agent exchanges ID-JAG at Auth0 → Auth0 access token  (sub=user)
    6. Agent performs OBO token exchange at Auth0 → delegated token  (sub=user, act.sub=agent_id)
    7. Page reloads showing all tokens + API response with user and agent identity
"""
...
from agent import get_id_jag_from_okta, get_auth0_access_token, get_obo_delegated_token, call_api
...
PROVISION_CALLBACK_URI = "http://localhost:5000/provision/callback"
...
def _exchange_code_for_id_token(code: str, verifier: str) -> dict:
...
def callback():
    ...
    print(f"\n{'='*60}")
    print(f"Callback received. Starting XAA + Agent as a Principal flow...")
    print(f"{'='*60}")
    ...
    try:
        ...
        # Step 3 — agent exchanges ID-JAG at Auth0 for access token (sub=user)
        ...
        # Step 4 — agent calls the protected API with the OBO token
        api_data = call_api(obo_token)
        print(f"\n{'='*60}")
        print("XAA + Agent as a Principal flow complete!")
        print(f"{'='*60}\n")
...
if __name__ == "__main__":
    print("XAA + Agent as a Principal UI running  → http://localhost:5000")
